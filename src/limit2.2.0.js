...
;(function(WIN){
	...
	const { toString, hasOwnProperty, valueOf } = objectProto;
	const { from, of } = Array;
	...
	// --错误日志-- //
		...
		// 错误提醒
		const typeWarn = {
			toString: (obj, objStr) => limit.message('warn', obj, 'change into', objStr),
			toNumber: obj => limit.message('warn', obj, 'change into', 'NaN'),
			toArray: obj => limit.message('warn', obj, 'change into []'),
			finiteNum: obj => limit.message('warn', obj, 'is not a finite Number'),
			formatDate: obj => limit.message('warn', 'timestamp:', timestamp, 'date:', date, 'limit.formatDate is called'),
			bind: obj => limit.message('warn', fun, 'type is not function, limit.bind is called')
		};
		...
		// 快捷方法
		defineIt('err', {
			value: (...args) => { cacheLog([`err[${limit.formatDate()}]`, ...args]), limit.message(...['error', ...args]) }
		});
		defineIt('war', {
			value: (...args) => { cacheLog([`war[${limit.formatDate()}]`, ...args]), limit.message(...['warn', ...args]) }
		});
		defineIt('inf', {
			value: (...args) => { cacheLog([`inf[${limit.formatDate()}]`, ...args]), limit.message(...['info', ...args]) }
		});
		defineIt('log', {
			value: (...args) => { limit.message(...['log', ...args]) }
		});
		// 把警告和错误，缓存到本地
		...
		const cacheClear = () => {
			if( winLocalStorage ){
				let time = limit.formatDate(new Date().getTime(), 'yyyyMMdd.HH');
				// 清楚1天之前的信息
				limit.forin(winLocalStorage, (val, key) => {
		...
		const cacheLog = (args) => {
			if( winLocalStorage ){
				// 获取当前的时间
				let time = limit.formatDate(new Date().getTime(), 'yyyyMMdd.HH');
				let cacheId = `limitLog${time}`;
				let oldLog = winLocalStorage.getItem(cacheId) || '';
				// 缓存信息
	...
	// --判定方法-- //
		...
		defineIt('isObjectSuper', { value: n => toString.call(n) === '[object Object]' && '' + n === '[object Object]' });
	...
	// --工具方法-- //
		...
		defineIt('getTimeUid', {
			value: () => new Date().getTime() + limit.getUid().split('.').join('')
		});
		...
		defineIt('_loop', {
			value: (obj, iterator, context, isBreak, begin) => {
				...
				let target = limit._getLoopKey(obj),
					key, num = limit.parseInt(begin), len = target.length;
				for(; num < len; num++){
					key = target[num];
	...
	// --函数-- //
		...
		defineIt('bind', {
			...
			fixed(fn, ...args1){
				...
				function main(...args2){
					...
					args1.shift();
					return Function.call.apply( fn, [me, ...args1, ...args2] );
				};
				classExtend(main, fn);
				// 咋骗，可以让兼容方法伪装的更像
		...
		class Events {
			...
			warnListenerCount(eventName){
				...
				let count = listeners[eventName].length;
	...
	// --字符-- //
		...
		defineIt('wordWrap', {
			format: checkTargetWithString,
			fixed( str, length = 0, key = '...' ){
				let totle = str.length;
		...
		defineIt('unParseString', {
			...
			fixed(obj, val1 = '&', val2 = '='){
				let list = [];
				limit.isObjectSuper(obj) && limit.each(obj, (val, key) => {
		...
		defineIt('serialize', {
			...
			fixed(obj){
				limit.each(obj, (val, key) => {
					// 对象规范
					if( REX_OBJ.test(key) ){
						let tempKey = RegExp.$1; //b
						let tempVal = RegExp.$2; //a
						let tempObj = obj[tempKey];
						if( !tempObj ){
							tempObj = obj[tempKey] = {};
						};
						tempObj[tempVal] = val;
					...
					}else if( REX_ARR.test(key) ){ //数组规范
						let tempKey = RegExp.$1; //b
						let tempPos = RegExp.$2; //0
						let tempVal = RegExp.$3; //a
						let tempArr = obj[tempKey];
						if( !tempArr ){
							tempArr = obj[tempKey] = [];
						};
						let tempObj = tempArr[tempPos];
						if( !tempObj ){
							tempObj = tempArr[tempPos] = {};
						};
						tempObj[tempVal] = val;
						delete obj[key];
						return limit.serialize(tempObj);
